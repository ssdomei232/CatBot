// source: github.com/bearbin/mcgorcon
...
func (r *RemoteConsole) Execute(cmd string) (string, error) {
	id, err := r.Write(cmd)
	if err != nil {
		return "", err
	}
	for {
		resp, rid, data, err := r.readResponse(10 * time.Second)
		if err != nil {
			return "", err
		}
		if resp == respResponse && rid == id {
